                    ...
                    const TypePublic = (props) => {
                        return (
                            <div className="type_p">
                                <p className="type_title">{props.title}</p>
                            </div>
                        )
                    }
                ```
            2. React.createClass(有状态函数)
                > React.createClass会自绑定函数方法
                > React.createClass的mixins不够自然、直观

            3. React.Component(有状态函数)
                > 可以更好实现代码复用

            * React.createClass与React.Component区别
                > this指向
                React.createClass创建的组件，其每一个成员函数的this都有React自动绑定，任何时候使用,函数中的this会被正确设置。
